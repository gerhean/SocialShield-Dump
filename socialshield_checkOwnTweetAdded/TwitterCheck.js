...
if (window == top) {
    chrome.runtime.onMessage.addListener(function (req, sender, sendResponse) {
        ...
        if (req.search){
            ...
            const checkName = title.substr(title.indexOf('(') + 2, title.indexOf(')') - title.indexOf('(') - 2);
            if (name != checkName){
                return
            }
        }
        ...
        if(req.selfCheck) {
            $('#doc').foggy(false);
            if (req.useCache){
                console.log('Use cache with profanity index', req.currentUrlCount);
                const banner = '<div id="ssBanner" style="height:50px"><h1 style="padding-top:65px;padding-bottom:20px;background-color:#EFEFEF;text-align:center;color:'+customSettingsForDanger[req.currentUrlCount].color+'">Threat Level: '+req.currentUrlCount+' / 5'+'</h1></div>'
                ...
                twitterScanDanger(name).then((profanityIndex) => {
                    console.log('profanity index of user', profanityIndex);
                    const banner = '<div id="ssBanner" style="height:50px"><h1 style="padding-top:65px;padding-bottom:20px;background-color:#EFEFEF;text-align:center;color:'+customSettingsForDanger[profanityIndex].color+'">Threat Level: '+profanityIndex+' / 5'+'</h1></div>'
                    if($('#ssBanner').length == 0) {
                        $('body').prepend(banner)
                    };
                ...
                parts.forEach((part) => {
                    ...
                    requestTweets("https://api.twitter.com/1.1/search/tweets.json", {
                        ...
                        if (phase == parts.length) {
                            console.log('friends count', friendsProfanityCount,'friends length', friends.length);
